...
},
{
  ...
  devolverLibro(title: string) {
    return this.libros.find(book => book.book.title === title);
  }

  minPaginas() {
    let minPaginas = this.libros[0].book.pages;
    for(const book of this.libros){
      if(book.book.pages < minPaginas) {
        minPaginas = book.book.pages;
      }
    }
    return minPaginas;
  }
  maxPaginas() {
    let maxPaginas = this.libros[0].book.pages;
    for(const book of this.libros){
      if(book.book.pages > maxPaginas) {
        maxPaginas = book.book.pages;
      }
    }
    return maxPaginas;
  }
